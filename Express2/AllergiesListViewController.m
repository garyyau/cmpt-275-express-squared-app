...
#import "AllergiesListViewController.h"

@interface AllergiesListViewController ()

@end

@implementation AllergiesListViewController
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section {
    ...
    footerLabel.text = [NSString stringWithFormat:@"%s", "Type in each of the allergies that the user has, separated by a comma between each one. WARNING: Pressing SAVE will save the current information on screen and any previous information will be overwritten."];
...
- (void)viewDidLoad {
    ...
    AllergiesTextField.text = AllergiesList;
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    if(buttonIndex==0) {
        ...
        NSString *AllergiesList = [AllergiesTextField text];
        ...
        NSLog(@"Data saved");
        [self performSegueWithIdentifier:@"SubmitAllergies" sender:self];
    }
}
