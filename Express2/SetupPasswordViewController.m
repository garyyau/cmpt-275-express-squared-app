...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section {
	...
	footerLabel.frame = CGRectMake(10.0, 0.0, 300.0, 90.0);
    footerLabel.numberOfLines = 10;
    
	// If you want to align the header text as centered
	// headerLabel.frame = CGRectMake(150.0, 0.0, 300.0, 44.0);
    
	footerLabel.text = [NSString stringWithFormat:@"%s", "The Password will be used by the Guardian(s) to access the Options Menu. To Reset the password, visit the Enter Password screen. To Change the password, visit the Change Password screen."]; // i.e. array element
...
- (void)viewDidLoad {
    ...
    WarnEnterPasswordLabel.hidden = TRUE;
    WarnConfirmPasswordLabel.hidden = TRUE;
    
    [super viewDidLoad];
    
    [NSTimer scheduledTimerWithTimeInterval:0.05 target:self selector:@selector(checkText) userInfo:nil repeats:YES];
...
- (void)checkText {
    if (EnterPasswordTextField.text.length > 4) {
        WarnEnterPasswordLabel.hidden = FALSE;
    } else {
        WarnEnterPasswordLabel.hidden = TRUE;
    }
    
    if (ConfirmPasswordTextField.text.length > 4) {
        WarnConfirmPasswordLabel.hidden = FALSE;
    } else {
        WarnConfirmPasswordLabel.hidden = TRUE;
    }
}
...
- (void)viewDidUnload {
    ...
    WarnEnterPasswordLabel = nil;
    WarnConfirmPasswordLabel = nil;
    NavigationBar = nil;
    [super viewDidUnload];
...
- (IBAction)NextButton:(id)sender {
    ...
    if (![EnterPassword isEqualToString:ConfirmPassword]) {
        ...
        [alert show];
    } else if (ConfirmPasswordTextField.text.length != 4) {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Password Length Error"
                                                        message:@"The password you entered is not 4-characters."
                                                       delegate:self
                                              cancelButtonTitle:@"OK"
                                              otherButtonTitles:nil];
        [alert show];
        ...
        [defaults setObject:EnterPassword forKey:@"SavedPassword"];
